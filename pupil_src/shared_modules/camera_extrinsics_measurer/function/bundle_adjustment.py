...
BundleAdjustmentResult = collections.namedtuple(
    "BundleAdjustmentResult", ["frame_id_to_extrinsics", "valid_key_marker_ids"]
)
...
class BundleAdjustment:
    def __init__(self, optimize_camera_intrinsics, optimize_marker_extrinsics=True):
        self._optimize_camera_intrinsics = optimize_camera_intrinsics
        self._optimize_marker_extrinsics = optimize_marker_extrinsics
        ...
        self._marker_ids = []
        self._frame_ids = []
        self._key_marker_ids = []

    def calculate(self, camera_intrinsics, initial_guess_result):
        ...
        camera_extrinsics_array, self._marker_extrinsics_array = self._set_init_array(
            initial_guess_result.frame_id_to_extrinsics,
            initial_guess_result.marker_id_to_extrinsics,
        )
    ...
    def _prepare_basic_data(self, key_markers):
        ...
        self._key_marker_ids = np.array([marker.index for marker in key_markers])
    ...
    def _prepare_parameters(self, camera_extrinsics_array, marker_extrinsics_array):
        ...
        if self._optimize_marker_extrinsics:
            initial_guess_array = np.vstack(
                (camera_extrinsics_array, marker_extrinsics_array)
            ).ravel()
        else:
            initial_guess_array = np.vstack(camera_extrinsics_array).ravel()
        ...
        bounds = self._calculate_bounds(marker_extrinsics_array)

        sparsity_matrix = self._construct_sparsity_matrix()

        return initial_guess_array, bounds, sparsity_matrix

    def _calculate_bounds(self, marker_extrinsics_array=None, eps=1e-15, scale=3e2):
        ...
        if self._optimize_marker_extrinsics:
            marker_extrinsics_lower_bound = np.full(
                self._marker_extrinsics_shape, -scale
            )
            ...
            lower_bound = camera_extrinsics_lower_bound.ravel()
            upper_bound = camera_extrinsics_upper_bound.ravel()
    ...
    def _construct_sparsity_matrix(self):
        ...
        if self._optimize_marker_extrinsics:
            ...
            sparsity_matrix = np.hstack((sparsity_matrix, mat_marker))
    ...
    def _get_result(self, least_sq_result):
        ...
        valid_key_marker_indices = self._find_valid_key_marker_indices(
            least_sq_result.fun, self._camera_intrinsics.resolution[0] / 135
        )
        ...
        bundle_adjustment_result = BundleAdjustmentResult(
            frame_id_to_extrinsics_opt, valid_key_marker_ids
        )
        return bundle_adjustment_result
    ...
    def _get_extrinsics_arrays(self, variables):
        ...
        if self._optimize_marker_extrinsics:
            ...
            marker_extrinsics_array = self._marker_extrinsics_array
    ...
    @staticmethod
    def _find_valid_key_marker_indices(residuals, thres):
        ...
        valid_key_marker_indices = np.where(reprojection_errors < thres)[0]
        return valid_key_marker_indices
